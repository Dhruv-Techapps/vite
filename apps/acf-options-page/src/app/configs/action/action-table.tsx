...
const ActionTable = ({ actions }: ActionProps) => {
  ...
  const columns = useMemo<ColumnDef<Action, ActionMeta>[]>(
    () => [
      ...
      },
      {
        header: () => (
          <>
            {t('action.elementFinder')} <small className="text-danger">*</small>{' '}
            <ElementFinderPopover />
          </>
        ),
        minSize: 400,
        maxSize: 1000,
  ...
  const table = useReactTable<Action>({
    ...
    meta: {
      ...
      updateValueFieldTypes: (
        selectedActionId: RANDOM_UUID,
        valueFieldType: 'input' | 'textarea'
      ) => {
        dispatch(
          updateAction({
            selectedActionId,
  ...
  const onDisableClick = (row: Row<Action>, disabled: boolean) => {
    dispatch(
      updateAction({
        selectedActionId: row.original.id,
        name: 'disabled',
        value: !disabled,
      })
    );
      ...
      <Table id="actions" hover className="mb-0">
                ...
                <Dropdown className="ml-2" id="acton-column-filter-wrapper">
                  ...
                  <Dropdown.Menu>
                    ...
                    <Dropdown.Item
                      ...
                      active={columnVisibility.initWait}
                    >
                      {t('action.initWait')}
                    </Dropdown.Item>
                    <Dropdown.Item
                      onClick={onColumnChange}
                      data-column="repeat"
                      active={columnVisibility.repeat}
                    >
                      {t('action.repeat')}
                    </Dropdown.Item>
                    <Dropdown.Item
                      ...
                      active={columnVisibility.repeatInterval}
                    >
        ...
        <tbody>
          {table.getRowModel().rows.map((row, index) => (
            <tr
              key={row.id}
              className={actions[row.id].disabled ? 'table-secondary' : ''}
            >
              ...
              <td align="center">
                {actions[row.id].disabled && (
                  <Ban className="me-2" title="Disabled" />
                )}
                <Button
                  variant="link"
                ...
                {actions[row.id].elementFinder && (
                  <Dropdown
                    id="acton-dropdown-wrapper"
                    className="d-inline-block"
                  >
                    <Dropdown.Toggle
                      as={DropdownToggle}
                      id="action-dropdown"
                      aria-label="Action more option"
                    >
                      <ThreeDots width="24" height="24" />
                    </Dropdown.Toggle>
                    <Dropdown.Menu>
                      <Dropdown.Item
                        data-testid="action-addon"
                        onClick={() => showAddon(row)}
                      >
                        {t('action.addon')}
                      </Dropdown.Item>
                      <Dropdown.Item
                        data-testid="action-settings"
                        onClick={() => showSettings(row)}
                      >
                        {t('action.settings')}
                      </Dropdown.Item>
                      {index !== 0 && (
                      ...
                      <Dropdown.Item
                        ...
                        onClick={() => onAddClick(row, 0)}
                      >
                        <Plus className="me-2" /> {t('action.addBefore')}
                      </Dropdown.Item>
                      <Dropdown.Item
                        data-testid="action-add"
                        onClick={() => onAddClick(row, 1)}
                      >
                      ...
                      <Dropdown.Item
                        ...
                        onClick={() =>
                          onDisableClick(row, actions[row.id].disabled)
                        }
                      >
                        {t(
                          `action.${
                            actions[row.id].disabled ? 'enable' : 'disable'
                          }`
                        )}
                      </Dropdown.Item>
                    </Dropdown.Menu>
