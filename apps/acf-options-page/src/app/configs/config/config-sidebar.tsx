    ...
    <div className="rounded sidebar bg-body">
      <div className="d-flex justify-content-between align-items-center border-bottom p-2 rounded-top">
        ...
        <Dropdown className="ml-2" id="config-detail-filter-wrapper">
          <Dropdown.Toggle
            as={DropdownToggle}
            id="configs-detail-filter"
            className="fs-4"
            aria-label="Toggle Action Column"
          >
            <EyeSlashFill />
          </Dropdown.Toggle>
          <Dropdown.Menu>
            <Dropdown.Item
              ...
              data-column="name"
              disabled={!detailVisibility.url}
              active={detailVisibility.name}
            >
            ...
            <Dropdown.Item
              onClick={onDetailChange}
              data-column="url"
              disabled={!detailVisibility.name}
              active={detailVisibility.url}
            >
              {t('configuration.url')}
            </Dropdown.Item>
          </Dropdown.Menu>
        ...
        <Dropdown id="configs-dropdown-wrapper">
          <Dropdown.Toggle
            as={DropdownToggle}
            id="configs-dropdown"
            aria-label="Configurations more option"
            data-testid="configurations-more-option"
            className="fs-4"
          >
            <ThreeDots />
          </Dropdown.Toggle>
          <Dropdown.Menu>
            <Dropdown.Item
              onClick={() => props.onExportAll(configs)}
              data-testid="configurations-export-all"
            >
              {t('configuration.exportAll')}
            </Dropdown.Item>
            <Dropdown.Item
              onClick={() => props.importFiled.current?.click()}
              data-testid="configurations-import-all"
            >
            ...
            <Dropdown.Divider />
            <Dropdown.Item
              onClick={() => dispatch(switchConfigReorderModal(configs))}
              data-testid="configurations-reorder-config"
            >
      ...
      <ListGroup as="ul" data-testid="configuration-list">
        {configs.length === 0 && (
          <ListGroup.Item as="li">{t('configuration.noConfig')}</ListGroup.Item>
        )}
        {configs.map((config, index) => (
          <ListGroup.Item
            ...
            action
            data-index={index}
            className={`d-flex justify-content-between ps-2 pe-0 ${
          ...
          >
            ...
            >
              <div className="w-100">
                {detailVisibility.name && (
                  <div className="text-truncate">{`${
                    config.name || 'configuration - ' + (index + 1)
                  }`}</div>
                )}
                {detailVisibility.url && (
              ...
              {!config.enable && (
                <Ban className="link-secondary" title="Disabled" />
              )}
            </div>
            <Button
              variant="link"
              data-testid="remove-configuration"
              onClick={(e) => onRemoveConfig(e, config)}
              disabled={configs.length === 1}
            >
              <Trash
                className={configs.length === 1 ? '' : 'link-danger'}
                title="Delete"
              />
            </Button>
          </ListGroup.Item>
