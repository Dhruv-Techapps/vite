    ...
    fr.onload = function ({ target }) {
      try {
        if (target?.result === null) {
          ...
          const importedConfigs: Array<Configuration> | Configuration =
            JSON.parse(target?.result as string);
            ...
            {props.errorButton && (
              <Alert.Link
                href={`${CHROME_WEB_STORE}${
                  import.meta.env.VITE_PUBLIC_CHROME_EXTENSION_ID
                }`}
                target="_blank"
                className="ms-2"
              >
